...
| ~~高~~ | ~~Common Voice DNN混合学習~~ | ~~GMM学習では失敗(-22pt)したがDNNは表現力が高い~~ → **不採用**: v21(TTS+CV 4,686発話)はval_acc 76.3%(-9.7pt)、tuner 72/90(-4)。TTS-GMMでCV音声をアライメントしたラベルの品質が低く、DNN学習を阻害 |
...
| 実験 | 発話数 | フレーム | val_acc | tuner最良 | vs v19 (76/90) |
|---|---|---|---|---|---|
| v19 (TTSのみ) | 5,794×5 | 3,309,827 | 86.0% | 76/90 (84.4%) | — |
| v21 (TTS+CV) | 5,794×5+4,686 | 4,604,672 | 76.3% | 72/90 (80.0%) | **-4** |

**分析**: val_accが86.0%→76.3%(-9.7pt)と大幅低下。TTS-GMMのアライメントがCV実音声に対して不正確で、ノイズラベルがDNN学習を阻害。GMM(-22pt)ほどの壊滅ではないがDNNでも-4の劣化。最適パラメータもLW=8,WP=8に低下(v19: LW=10,WP=10)し、音響モデルの信頼度自体が下がっている。CV実音声活用にはCV音声に適応したGMM(またはDNN自身によるself-training/iterative alignment)が必要。
