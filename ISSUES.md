...
| **高** | **DNN層幅拡大 (512→768)** | **パラメータ増で弁別力向上。val_acc 86%→87%+期待。フラグ変更のみ** |
| **高** | **コンテキスト窓拡大 (±5→±7)** | **入力429→585次元。遷移区間・短語の認識改善。フラグ変更のみ** |
...
#### Phase 1: DNNアーキテクチャ拡張 (低コスト、+1~2期待)

フラグ変更のみで再学習・再評価。

1. **DNN 768unit化**: `-hidden 768` (現在512)。パラメータ 1.58M→~2.4M。弁別力向上
2. **コンテキスト窓拡大**: `-context 7` (現在5)。入力429→585次元。遷移区間・短語の認識に有利
3. 上記を組み合わせて v20 として学習、tunerで再チューニング
4. 速度変換9-way (0.85~1.15) も同時に試行
...
- Phase 1はフラグ変更のみで実装コストゼロ。val_acc向上→認識精度向上の実績がv14→v19で一貫しているため、アーキテクチャ拡大は最も確実
- Phase 2はCommon Voice失敗(GMM)の逆転打。DNN-HMMハイブリッドでは実音声混合の成功例が多い (Kaldi等)
