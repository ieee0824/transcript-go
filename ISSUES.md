...
| **高** | **Common Voice DNN混合学習** | **GMM学習では失敗(-22pt)したがDNNは表現力が高い。TTS-GMMアライメントでCV音声のフレームラベル生成→DNN学習データに混合** |
...
#### Phase 2: 学習データ拡張 (中コスト、+1~2期待)

5. **Common Voice DNN学習**: TTS-GMMでCV音声をアライメント→DNNの学習データに混合。GMMでは分布拡散で-22ptだったが、DNNの表現力ならTTS+実音声の共存が可能な可能性
...
- Phase 2はCommon Voice失敗(GMM)の逆転打。DNN-HMMハイブリッドでは実音声混合の成功例が多い (Kaldi等)。**Phase 1失敗によりデータ拡張が最優先に**
